...
class AddModal extends React.Component {
    ...
    async DatHang(){
        const url = 'http://tittee.000webhostapp.com/tee/dathang.php?name=' + this.state.name + '&sdt=' + this.state.phoneNumber + '&diaChi=' + this.state.address + '&soluong=' + this.state.sl+'&idHang='+this.props.item.id;
    ...
    render() {
        return (
            <Modal style={{ justifyContent: 'center', borderRadius: 4, width: Dimensions.get('window').width - 80, height: 350 }}
                ref={"myModal"}
                position='center'
                backdrop={true}
                onClosed={() => {
            ...
            >
                ...
                <View style={{ height: 40,flexDirection: 'row',marginBottom:20 }}>
                    ...
                    <TouchableOpacity onPress={() => { this.DatHang() }} style={{ borderRadius: 4, backgroundColor: 'gray', flex: 1 }}>
